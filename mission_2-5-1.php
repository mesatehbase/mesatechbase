<html>
<head>
<title>form2-5-1</title>
<meta charset="UTF-8 ">
</head>
</html>
<?php
$filename='mission_2-5-1.txt';
...
if(file_exists($filename)){
	$hairetu=file("mission_2-5-1.txt");
...
if($new_line[0]!=$delete){
	$b=fopen($filename, "a");
            fwrite($b, $delC);
            fclose($b);
        } elseif(($new_line[0] == $delete)&&($new_line[4]==$_POST["password"])){
            $c = fopen($filename, "a");
            fwrite($c, "消去しました。\n");
            fclose($c);
        }
...
if(!empty($edit)){
	...
	foreach($d as $f){
	$new_line=(explode("<>",$f));
if(($new_line[0]==$edit)&&($new_line[4]==$_POST["password"])){
	$data1=$new_line[1];
	$data2=$new_line[2];
	$data3=$new_line[3];
}	
